...
public class EvaluateSelfExpressionTest {
	private static final String PATH = "org.osate.contract.tests/models/interpreter/EvaluateSelfExpressionTests/";
	...
	@Test
	public void testSelfExpression() throws Exception {
		var pkg = testHelper.parseFile(PATH + "self_expression_test.aadl");
		var system = (SystemImplementation) pkg.getPublicSection().getOwnedClassifiers().get(1);
		var systemInstance = InstantiateModel.instantiate(system);
		var sub = systemInstance.getComponentInstances().get(0);
		var environment = new RuleEnvironment(new RuleEnvironmentEntry("self", sub));
		...
		assertEquals("sub", ((ComponentInstance) result.get("v1")).getName());
	}
}