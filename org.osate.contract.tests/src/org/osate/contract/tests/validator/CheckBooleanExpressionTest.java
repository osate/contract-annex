...
public class CheckBooleanExpressionTest {
	private static final String PATH = "org.osate.contract.tests/models/validator/CheckBooleanExpressionTests/";
	...
	@Test
	public void testWrongTypesForBooleanExpression() {
		var pkg = testHelper.parseFile(PATH + "wrong_types_for_boolean_expression.aadl");
		...
		with(issues.get(0), issue -> {
			...
			assertEquals("The operator || is undefined for the argument types ComponentInstance, String",
					issue.getMessage());
			assertEquals(EcoreUtil.getURI(contract.getQueries().get(0).getValue()), issue.getUriToProblem());
		});
		with(issues.get(1), issue -> {
			assertEquals(Severity.ERROR, issue.getSeverity());
			assertEquals("The operator && is undefined for the argument types Boolean, ComponentCategory",
					issue.getMessage());
			assertEquals(EcoreUtil.getURI(contract.getQueries().get(1).getValue()), issue.getUriToProblem());
		});
		with(issues.get(2), issue -> {
			assertEquals(Severity.ERROR, issue.getSeverity());
			assertEquals("The operator ! is undefined for the argument type List<ComponentInstance>",
					issue.getMessage());
