...
public class CheckTupleDeclarationSizeTest {
	private static final String PATH = "org.osate.contract.tests/models/validator/CheckTupleDeclarationSizeTests/";
	...
	public void testWrongCount() {
		...
		with(issues.get(0), issue -> {
			...
			assertEquals("Expected tuple with 2 elements; found tuple with 3 elements", issue.getMessage());
			assertEquals(EcoreUtil.getURI(contract.getQueries().get(1)), issue.getUriToProblem());
		});
		with(issues.get(1), issue -> {
			assertEquals(Severity.ERROR, issue.getSeverity());
			assertEquals("Expected tuple with 3 elements; found tuple with 2 elements", issue.getMessage());
	...
	public void testNotATuple() {
		...
		with(issues.get(0), issue -> {
			assertEquals(Severity.ERROR, issue.getSeverity());
			assertEquals("Expected tuple; found ComponentInstance", issue.getMessage());
			assertEquals(EcoreUtil.getURI(contract.getQueries().get(1)), issue.getUriToProblem());
		});
	}
}