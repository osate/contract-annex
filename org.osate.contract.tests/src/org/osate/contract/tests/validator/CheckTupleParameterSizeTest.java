...
public class CheckTupleParameterSizeTest {
	...
	public void testWrongCount() {
		...
		with(issues.get(2), issue -> {
			...
			assertEquals(
					EcoreUtil.getURI(
							((MemberCall) ((SingleValDeclaration) contract.getQueries().get(4)).getValue()).getLambda()
									.getParameter()),
					issue.getUriToProblem());
		});
		with(issues.get(3), issue -> {
			...
			assertEquals(
					EcoreUtil.getURI(
							((MemberCall) ((SingleValDeclaration) contract.getQueries().get(5)).getValue()).getLambda()
									.getParameter()),
					issue.getUriToProblem());
		});
	}
